...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 11) and (File.idSample = Sample.id) ')
dfCounts = dbGetQuery(db, q)
head(dfCounts)
# # since the files are trimmed, add the prefix trim
# dfCounts$name = paste('trim_', dfCounts$name, sep='')
nrow(dfCounts)
dfCounts
...
cvGeneIndex = '/users/k1625253/brc_scratch/Data/MetaData/GenomeIndex/mm10_hisat2'
