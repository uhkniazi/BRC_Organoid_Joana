...
setwd(gcswd)
n = make.names(paste('CFastqQuality organoids rna_seq rds'))
lOb$meta = dfSample
lOb$desc = paste('CFastqQuality object from organoids rna_seq run', date())
...
# df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/', comment='organoids project FASTQ file quality data')
...
barplot(iReadCount, las=2, main='Pre-Trim Read Count', ylab = 'No. of Reads in Millions', cex.names =0.6, col=grey.colors(2))
...
plot(hc, main='Clustering of Pre-trim data based on Per base quality', cex=0.6, xlab='', sub='')
abline(h = 18, col=2)
